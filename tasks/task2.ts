...
const getNormalizeData = (data: PostData[]): NormalizeData => {
  const normalizeData: NormalizeData = {
    byId: {},
    allIds: [],
  };

  data.forEach(({ body, id, title }) => {
    normalizeData.allIds.push(id);
    normalizeData.byId[id] = { body, id, title };
  });
  return normalizeData;
};

console.log(getNormalizeData(posts));
